...
const InterviewerCard = forwardRef<InterviewerCardHandle, InterviewerCardProps>(
  ({ interviewer, isActive, sessionId, onSpeakingChange }, ref) => {
    ...
    return (
      <div
        className={`relative rounded-xl overflow-hidden bg-zinc-900 border-2 transition-colors duration-300 w-full aspect-square md:aspect-auto md:h-full ${
          isSpeakingNow
      ...
      >
        ...
        <div
          className={`absolute top-2 right-2 flex items-center gap-1.5 bg-black/70 border border-primary/40 rounded-md px-2 py-1 backdrop-blur-sm transition-opacity duration-300 pointer-events-none ${
            isSpeakingNow ? "opacity-100" : "opacity-0"
          }`}
        >
