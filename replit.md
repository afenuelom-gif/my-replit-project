...
- **Pricing / payments**: Stripe is fully integrated. Starter ($12/mo) and Pro ($24/mo) subscriptions and top-up packs use real Stripe Checkout. Backend uses plain `stripe` SDK with `STRIPE_SECRET_KEY` env var. Webhook handling at `/api/stripe/webhook` requires `STRIPE_WEBHOOK_SECRET`. The `sync-user-plan` endpoint reads subscription state directly from Stripe API after checkout redirect. Products/prices are hardcoded in `pricing.tsx` (see scratchpad for IDs). Webhook secret can be added later via Stripe dashboard → Destinations → add webhook URL.
...
- **Transactional emails** (`emailService.ts`): Powered by Resend (`RESEND_API_KEY` secret). Sends: welcome on first sign-in, subscription confirmed/cancelled, payment failed, top-up credits confirmed, low session credits nudge (at 1 remaining), low tailor credits nudge (at 1 remaining). All sends are fire-and-forget (non-blocking). **Domain setup required**: verify `prepinterv.com` in the Resend dashboard (Settings → Domains) so emails send from `noreply@prepinterv.com`. Override the from address via `EMAIL_FROM` env var or the app URL via `APP_URL`.
- **Admin dashboard** (`/admin`): 4 admin pages — Feedback, Users, Tailors, Revenue. Revenue page (`/admin/revenue`) pulls live data from Stripe: MRR, total revenue (split by subscriptions vs top-ups), active subscriber count (Starter/Pro breakdown), churn (30-day canceled subs), a 12-month revenue bar chart, and a recent payments table. Endpoint: `GET /api/users/admin/revenue`.
